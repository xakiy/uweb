...
from webapps.home.models import CopyrightText, SocialMedia
...
@register.inclusion_tag("components/copyright_text.html", takes_context=True)
def get_copyright_text(context):
    # Get the copyright text from the context if exists,
    # so that it's possible to pass a custom instance e.g. for previews
    # or page types that need a custom copyright
    copyright_text = context.get("copyright_text", "")

    # If the context doesn't have copyright_text defined, get one that's live
    if not copyright_text:
        instance = CopyrightText.objects.filter(live=True).first()
        copyright_text = instance.body if instance else ""
...
@register.inclusion_tag("components/social_media_link.html")
def get_social_media_link():

    socmeds = SocialMedia.objects.values()
    if socmeds:
        media = socmeds[0]
        media.pop('id')

        return {
            "social_media": {key:value for key, value in media.items() if value}
        }
    else:
        return None
